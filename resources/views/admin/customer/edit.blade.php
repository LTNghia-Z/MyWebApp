...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
            <h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-edit me-2"></i> Chỉnh sửa thông tin Khách hàng</h6>
        </div>
        <!-- Card Body -->
        <div class="card-body">
            <form action="{{ route('admin.customer.update', $customer->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('username') is-invalid @enderror" id="username" name="username" placeholder="Nhập tên khách hàng" value="{{ old('username', $customer->username) }}" required>
                    @error('username')
                ...
                <div class="mb-3">
                    ...
                    <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" name="email" placeholder="Nhập địa chỉ email" value="{{ old('email', $customer->email) }}">
                    @error('email')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('phone') is-invalid @enderror" id="phone" name="phone" placeholder="Nhập số điện thoại" value="{{ old('phone', $customer->phone) }}" required>
                    @error('phone')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mt-4">
                    <button type="submit" class="btn btn-warning shadow-sm">
                        <i class="fas fa-save fa-sm text-white-50 me-1"></i> Cập nhật Khách hàng
                    </button>
...
@php
    $addressJson = $customer->address ? json_decode($customer->address, true) : null;
@endphp

<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
<script>
    window.oldAddress = <?php echo json_encode(json_decode($customer->address, true)); ?>;
</script>
