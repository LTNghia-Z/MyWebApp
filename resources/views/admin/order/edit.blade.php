...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
            <h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-edit me-2"></i> Chỉnh sửa thông tin Đơn hàng</h6>
        </div>
        <!-- Card Body -->
        <div class="card-body">
            <form action="{{ route('admin.order.update', $order->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <div id="food-list">
                        @if(old('foods'))
                            @foreach(old('foods') as $index => $oldFoodId)
                                <div class="d-flex align-items-center mb-2 food-item">
                                    <select name="foods[]" class="form-control food-select">
                                        <option value="">-- Chọn món ăn --</option>
                                        @foreach ($foods as $food)
                                            <option value="{{ $food->id }}" data-price="{{ $food->food_price }}" {{ $oldFoodId == $food->id ? 'selected' : '' }}>
                                                {{ $food->food_name }} ({{ number_format($food->food_price, 0, ',', '.') }} VNĐ)
                                            </option>
                                        @endforeach
                                    </select>
                                    <input type="number" name="quantities[]" class="form-control ms-2 quantity-input" min="1" value="{{ old('quantities')[$index] ?? 1 }}">
                            ...
                            @foreach ($order->orderItems as $orderItem)
                                <div class="d-flex align-items-center mb-2 food-item">
                                    <select name="foods[]" class="form-control food-select">
                                        <option value="">-- Chọn món ăn --</option>
                                        @foreach ($foods as $food)
                                            <option value="{{ $food->id }}" data-price="{{ $food->food_price }}" {{ $orderItem->food_id == $food->id ? 'selected' : '' }}>
                                                {{ $food->food_name }} ({{ number_format($food->food_price, 0, ',', '.') }} VNĐ)
                                            </option>
                                        @endforeach
                                    </select>
                                    <input type="number" name="quantities[]" class="form-control ms-2 quantity-input" min="1" value="{{ $orderItem->quantity }}">
                                    <button type="button" class="btn btn-danger ms-2 remove-food"><i class="fas fa-trash"></i></button>
                                </div>
                            @endforeach
                        @endif
                        <div class="d-flex align-items-center mb-2 food-item" style="display:none" id="default-food-item">
                            <select name="foods[]" class="form-control food-select">
                                <option value="">-- Chọn món ăn --</option>
                                @foreach ($foods as $food)
                            ...
                            <input type="number" name="quantities[]" class="form-control ms-2 quantity-input" min="1" value="1">
                            <button type="button" class="btn btn-danger ms-2 remove-food"><i class="fas fa-trash"></i></button>
                        </div>
                    </div>
                    <button type="button" id="add-food" class="btn btn-primary mt-2"><i class="fas fa-plus"></i> Thêm món</button>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div id="total_price_display" class="form-control">0 VNĐ</div>
                ...
                <div class="mb-3">
                    ...
                    <select name="status" id="status" class="form-control @error('status') is-invalid @enderror" required>
                        ...
                        <option value="delivering" {{ old('status', $order->status) == 'delivering' ? 'selected' : '' }}>Delivering</option>
                        <option value="completed" {{ old('status', $order->status) == 'completed' ? 'selected' : '' }}>Completed</option>
                        <option value="cancelled" {{ old('status', $order->status) == 'cancelled' ? 'selected' : '' }}>Canceled</option>
                    </select>
                    @error('status')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="phone" id="phone" class="form-control @error('phone') is-invalid @enderror" value="{{ old('phone', $order->phone) }}" placeholder="Nhập số điện thoại" required>
                    @error('phone')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mt-4">
                    <button type="submit" class="btn btn-warning shadow-sm">
                        <i class="fas fa-save fa-sm text-white-50 me-1"></i> Cập nhật Đơn hàng
                    </button>
...
<script>
    $(document).ready(function() {
        function updateTotalPrice() {
            ...
            $('#total_price_display').text(total.toLocaleString('vi-VN') + ' VNĐ');
        }
...
<script> 
window.oldAddress = <?php echo json_encode($order->address, true); ?>;
</script>
